...
export default function UsersAdmin(){
 ...
 {u&&<Card title={`${u.name||u.username} 계정 수정`} desc={msg||'현재 비밀번호는 보안상 조회할 수 없으며, 관리자는 새 비밀번호로 재설정할 수 있습니다.'}><div className="adminFormGrid">
  ...
  <Field label="성별"><Input value={u.gender||''} onChange={e=>set('gender',e.target.value)}/></Field><Field label="공개 이메일"><Input value={u.public_email||''} onChange={e=>set('public_email',e.target.value)}/></Field>
  ...
  <Field label="유튜브 썸네일 URL"><Input value={u.youtube_thumbnail_url||''} onChange={e=>set('youtube_thumbnail_url',e.target.value)}/></Field><Field label="해시태그"><Input value={(u.tags||[]).join(', ')} onChange={e=>set('tags',e.target.value.split(',').map(v=>v.trim()).filter(Boolean))}/></Field>
