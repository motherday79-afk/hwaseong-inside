...
export default function NewsAdmin(){
 ...
 return <div className="adminPage"><PageTitle title="공지 & 뉴스 관리" desc="메인 카드와 게시판에 노출되는 게시물을 등록·수정·공개할 수 있습니다." actions={<button className="adminBtn primary" onClick={add}>+ 새 게시물</button>}/>
  ...
  {current&&<Card title="게시물 수정" desc="대표이미지는 메인과 게시판에서 같은 원본을 사용합니다."><ImageUploader label="대표 이미지" value={current.coverUrl} onChange={v=>update({coverUrl:v})} spec={IMAGE_SPECS.newsCover}/><div className="adminFormGrid"><Field label="카테고리"><Input value={current.category} onChange={e=>update({category:e.target.value})}/></Field><Field label="게시일"><Input value={current.date} onChange={e=>update({date:e.target.value})}/></Field><Field label="제목"><Input value={current.title} onChange={e=>update({title:e.target.value})}/></Field><Field label="요약"><Textarea rows={5} value={current.excerpt} onChange={e=>update({excerpt:e.target.value})}/></Field><Field label="본문"><Textarea rows={12} value={current.body||''} onChange={e=>update({body:e.target.value})}/></Field><Field label="공개 상태"><select className="adminInput" value={current.published?'public':'private'} onChange={e=>update({published:e.target.value==='public'})}><option value="public">공개</option><option value="private">비공개</option></select></Field></div><button className="adminBtn danger" onClick={del}>게시물 삭제</button></Card>}</div>
 </div>
}
