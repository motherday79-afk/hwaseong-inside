...
const strengthLabels=['분석','공감','연결','성장','창의','실행'];
...
export default function AccountClient({user}){
 ...
 useEffect(()=>{fetch('/api/profile').then(r=>r.json()).then(d=>{const member=d.member||{};member.careers=(member.careers||[]).sort((a,b)=>(a.sort_order||0)-(b.sort_order||0));member.strengths={...defaultStrengths,...(member.strengths||{})};setM(member)});},[]);
