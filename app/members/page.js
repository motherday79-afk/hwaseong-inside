...
export default async function MembersPage(){
 ...
 return <main className="publicListPage"><div className="publicPageHead"><a href="/">← 화성인사이드</a><h1>화성인사이드 멤버</h1><p>각 분야의 전문성을 가진 멤버들을 만나보세요.</p></div><div className="publicMemberGrid">{members.map(m=>{const src=clean(m.profile_image_url,m.id);return <a href={`/members/${m.id}`} key={m.id}><div>{src?<img src={src} alt={`${m.name} 프로필`}/>:<span>{(m.name||'?').slice(0,1)}</span>}</div><b>{m.name}</b><small>{m.job}</small><em>{m.tier} · {m.area}</em></a>})}</div></main>
}
