...
export default function Hero({user=null}){
  ...
  return <section className="v7Hero" id="about"><div className="v7Wrap v7HeroGrid">
    <div className="v7HeroCopy"><h1><span>{s.heroTitle1}</span><br/>{s.heroTitle2}</h1><p>{s.heroDescription}</p><div className="v7HeroActions"><a className="v7Btn v7BtnPrimary" href={primaryHref}><BrandIcon name={user?'gem':'login'} size={17}/>{primaryLabel}</a><a className="v7Btn" href="/notice-news">{s.secondaryButton} <BrandIcon name="arrowRight" size={16}/></a></div></div>
