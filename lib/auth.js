...
export async function getCurrentUser(){
  ...
  const [{data:account},{data:member}]=await Promise.all([
    admin.from('app_users').select('username,role,status').eq('user_id',user.id).maybeSingle(),
    admin.from('members').select('id,tier').eq('user_id',user.id).maybeSingle()
  ]);
  return {...user,username:account?.username||'',role:account?.role||'member',status:account?.status||'active',memberId:member?.id||null,tier:member?.tier||'Gold'};
}
