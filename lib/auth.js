...
export async function getCurrentUser(){
  ...
  const {data:account}=await admin.from('app_users').select('role,status').eq('user_id',user.id).maybeSingle();
  return {...user,role:account?.role||'member',status:account?.status||'active'};
}
